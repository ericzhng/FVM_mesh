"""
Mesh reordering tools for optimizing matrix bandwidth.

This module provides functions for reordering cells and nodes in a mesh to optimize
the bandwidth of the mesh's adjacency matrix. Reducing the bandwidth can
significantly improve the performance of numerical solvers by improving cache
...
class _GPSStrategy(_CellReorderStrategy):
    ...
    def get_order(self, mesh: CoreMesh) -> np.ndarray:
        ...
        adj_matrix = _get_adjacency(mesh)
        if not hasattr(adj_matrix, "shape") or adj_matrix.shape is None:
...
def renumber_nodes(mesh: CoreMesh, strategy: str = "rcm") -> None:
    ...
    new_order = strategy_map[strategy]()
